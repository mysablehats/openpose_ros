...
///this is not the best. I should try to use Float32MultiArray
...
// measuring time
    ...
    ros::NodeHandle nh;
            ros::Time begin = ros::Time::now(), endt;
    image_transport::ImageTransport it(nh);
    pub = it.advertise("image", 1);
    spub = nh.advertise<geometry_msgs::Polygon>("my_topic", 1);
    apub = nh.advertise<std_msgs::Float32MultiArray>("multi_skel", 1);
  ...
  //I'm preinitializing everything I can for speed's sake.
    ...
    std_msgs::Float32MultiArray skel_array_msg;
    ROS_INFO("Defined everything ready to acquire.");
    op::Array<float> poseKeypoints;
    std::vector<op::Array<float> > netInputArray ;
    int counter = 0;
    //if I have a constant size image maybe I can make netInputArray constant and pre initialize it for speed gain
    while(nh.ok()){
      begin = ros::Time::now();
      if (cv_ptr){
          ...
          netInputArray = cvMatToOpInput.createArray(cv_ptr->image, scaleInputToNetInputs, netInputSizes);
          // Step 4 - Estimate poseKeypoints
          poseExtractorCaffe.forwardPass(netInputArray, imageSize, scaleInputToNetInputs);
          // const op::Array<float> poses; //magic??
          // int num_people = poses.getSize(0);
          // ROS_INFO("found %d people", num_people);
          poseKeypoints = poseExtractorCaffe.getPoseKeypoints(); //probably an op::Array<float>
          ...
          //since the number of people change, I need to declare this every time. sad times for
          msg_dim.label = "person";
          msg_dim.size = num_people2;
          msg_dim.stride = 3*num_bodyparts*num_people2;
          skel_array_msg.layout.dim.clear();
          skel_array_msg.layout.dim.push_back(msg_dim);

          msg_dim.label = "bodyparts";
          msg_dim.size = num_bodyparts;
          msg_dim.stride = 3*num_bodyparts;
          skel_array_msg.layout.dim.push_back(msg_dim);

          msg_dim.label = "xyconfidence";
          msg_dim.size = 3;
          msg_dim.stride = 3;
          skel_array_msg.layout.dim.push_back(msg_dim);

          for (size_t nump = 0; nump < num_people2; nump++)
          {
            for (size_t bpart = 0; bpart < num_bodyparts*3; bpart+=3)
            {
              skel_array_msg.data.push_back(poseKeypoints[nump*num_bodyparts*3+bpart]);
              skel_array_msg.data.push_back(poseKeypoints[nump*num_bodyparts*3+bpart+1]);
              skel_array_msg.data.push_back(poseKeypoints[nump*num_bodyparts*3+bpart+2]); // or 0;
              // myperson.points.push_back(part);
            }
          }
          apub.publish(skel_array_msg);
          // after publishing this we should clear it or it get wrong. and big.
          skel_array_msg.data.clear();
        ...
        endt = ros::Time::now();
        double seconds = endt.toSec() - begin.toSec();
        ROS_INFO("time elapsed %f",seconds);
        ROS_INFO("fps %f",1/seconds);
              // counter++; /// i want to add a frame only if i got to this point. but no, wait,,,
        ros::spinOnce();
        r.sleep();
      }
